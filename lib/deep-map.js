...
const deepMap = (
  input, handler = v => v, path = ['$'], seen = new Set([input]), parent = undefined
) => {
  ...
  if (isRoot && input instanceof Error) {
    return deepMap({ err: filterError(input) }, handler, path, seen, input)
  }
  if (input instanceof Error) {
    return deepMap(filterError(input), handler, path, seen, input)
  }
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      if (element instanceof Object) {
        if (!seen.has(element)) { // avoid getting stuck in circular reference
          seen.add(element)
          result.push(deepMap(handler(element, elementPath), handler, elementPath, seen, input))
        }
      } else {
        ...
        if (property instanceof Object) {
          if (!seen.has(property)) { // avoid getting stuck in circular reference
            ...
            result[propertyName] = deepMap(
              handler(property, propertyPath), handler, propertyPath, seen, input
            )
          }
        } else {
